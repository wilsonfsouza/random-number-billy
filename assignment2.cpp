...
int main() {
  ...
  int guesses = INITIAL_GUESSES;  // number of user guesses initalize to 5 (give
                                  // users 5 guesses)
  ...
  while (!win && guesses > 0) {
    ...
    if (!isValidGuess(useNum)) {
      cout << "The number: " << useNum
           << " you enter is out of the range. Please re-enter a number "
              "between 1 and "
           << MAX_NUMBER << ": \n";
      continue;
    }
